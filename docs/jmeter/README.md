...
- **CRUD (обычные запросы):** достаточно `READ COMMITTED` — операции используют новые ID и не опираются на повторное чтение. В сервисе выставлен `REPEATABLE READ`, чтобы снизить число конфликтов сериализации при нагрузке и при этом сохранить защиту от фантомов.
